...
export async function PUT(req, { params }) {
  try {
    ...
    return NextResponse.json({ message: "Household updated", household: updatedHousehold }, { status: 200 });
  } catch (error) {
    return NextResponse.json({ error: "Error updating household" }, { status: 500 });
  }
}
...
export async function DELETE(req, { params }) {
  try {
    ...
    for (const user of users) {
      user.households = user.households.filter(id => id.toString() !== householdId.toString());
      await user.save();
    }
    ...
    return NextResponse.json({ error: "Error deleting household" }, { status: 500 });
  }
}
